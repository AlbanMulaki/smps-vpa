...
<style>
    ...
    .content{margin: 50px; margin-top:10px;}
    table,tr,td,th{
        text-align: left;
    }
    table{
...
@section('header')
<center style="line-height: 0.7;">
    {{ HTML::image('/img/'.$logo, 'a picture',array('style'=>'width:130px')) }}
    <p style="font-size:14px; line-height: 0.7em; font-weight: bold;">{{ $options['name_company'] }}</p>
    <p style="font-size:10px; line-height: 0.7em;">{{ $options['adressa'] }} {{ Lang::get('general.phone') }}: {{ $options['phone'] }}<p>
    <p style="font-size:10px; line-height: 0.7em;">{{ Lang::get('general.website') }}: {{ $options['website'] }},  {{ Lang::get('general.email') }}: {{ $options['info_email'] }},{{ $options['support_email'] }}<p>
</center>
<center>
    <h2>{{ $title }}</h2>
</center>
