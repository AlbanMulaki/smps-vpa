...
<div class="col-sm-12 btn btn-sm btn-block btn-default ">
    <a href="{{ action('ProfessorController@getRead') }}/{{ $value['id'] }}" value="44" id="lin44">
...
@section('compose')
{{ Form::open(array('url'=>action('ProfessorController@postSend'),'method'=>'POST','class'=>' form-horizontal','id'=>'form1searchcompose','role'=>'search','name'=>'form1searchcompose')) }}
...
<script>
    ...
    $('#searchcompose').click(function() {
        ...
        $.ajax({
            type: "POST",
            url: "{{ action('SearchPersonController@postIndex') }}",
            data: dataString,
            error: function() {
    ...
    $('#form1searchcompose').submit(function() {
        var dataString = $('#form1searchcompose').serialize();

        $.ajax({
            type: "POST",
            url: "{{ action('ProfessorController@postSend') }}",
            data: dataString,
            error: function() {
                alert('Something went go wrong');
            },
    ...
    $('#searchcompose').mouseenter(function() {
        var dataString = $('#form1searchcompose').serialize();
        $.ajax({
            type: "POST",
            url: "{{ action('SearchPersonController@postIndex') }}",
...
<div class='container well'>
    <div id='notification'>{{ Session::get('notification') }}</div>
